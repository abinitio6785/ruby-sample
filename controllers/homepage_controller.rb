class HomepageController < ApplicationController
  ...
  def upload_attachments
    attachment = MessageAttachment.create(file: params[:file])
    if attachment.save
      respond_to do |format|
        format.json { render json: attachment.to_jq_upload }
      end
    else
  ...
  def upload_listing_package_attachments
    attachment = ListingPackageAttachment.create(file: params[:file])
    if attachment.save
      respond_to do |format|
        format.json { render json: attachment.to_jq_upload }
      end
