class MessagesController < ApplicationController
  ...
  def create
    ...
    if @message.save
      if params.permit![:message_attachment_ids].present?
        params[:message_attachment_ids].each do |attachment_id|
          attachment = MessageAttachment.find(attachment_id)
          attachment.update_column(:message_id, @message.id) if attachment.present?
        end
      end
