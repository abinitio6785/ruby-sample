class PeopleController < Devise::RegistrationsController
  ...
  before_action :influencer_profile, :only => [:show]
  ...
  def influencer_profile
    if @current_user && @current_user.Seller?
      going_path = @current_user.listings.present? ? listing_path(@current_user.listings.last) : new_listing_path
      redirect_to going_path
    end
  end
  ...
  def show
    ...
    if @service.person.Seller?
      @listing = @service.listings.present? ? @current_community.listings.find(@service.person.listings.last.id) : nil
      render "seller_profile"
    else
      render "buyer_profile"
    end
  end
  ...
  def update_influencer_profile
    ...
    @target_user = Person.find_by!(id: params[:id], community_id: @current_community.id)
    puts(params)
    puts(@target_user.inspect)
    #save_categories(params[:listing_category])

    @target_user.categories.destroy_all
    params[:people_category].each do |category_id|
      @target_user.people_categories.build.update(category_id: category_id)
    end
  end
  ...
  def update
    ...
    if secondary_email_not_valid(params)
      flash[:error] = t("people.new.email_is_used_or_not_allow")
      redirect_back(fallback_location: homepage_url) and return
    end

    if params[:person][:secondary_email].present?
      email_address = target_user.emails.where(address: params[:person][:secondary_email]).present?
      unless email_address
        if target_user.emails.where(secondary_email: true).present?
          email = target_user.emails.where(secondary_email: true).last
          email.update(address: params[:person][:secondary_email])
          Email.send_confirmation(email, @current_community)
        else
          email = target_user.emails.new(address:  params[:person][:secondary_email], community_id: @current_community.id, secondary_email: true)
          if email.save
            Email.send_confirmation(email, @current_community)
          else
            flash[:error] = "something went wrong with this secondary email!!!"
            redirect_back(fallback_location: homepage_url) and return
          end
        end
      end
    ...
    begin
      ...
      if target_user.custom_update(person_params.except(:email_attributes))
        ...
        m_email_address.each {
          ...
          Email.send_confirmation(target_user.emails.primary_emails.last, @current_community)
        }
    ...
    if params[:referer_form] == 'settings' && target_user.errors.empty?
      if target_user.Seller?
        going_path = target_user.listings.present? ? edit_listing_path(target_user.listings.last) : new_listing_path
        redirect_to going_path
      else
  ...
  def person_update_permit
    [
      :country_code, :gender, :date_of_birth, :secondary_email,:one_line_desciption,:tiktok_link, :facebook_link, :instagram_link, :cover_photo,
      :password,
      :password2,
      :first_name, :last_name, :bank_id, :branch, :account_number, :account_type, :identification,
  ...
  def secondary_email_not_valid(params)
    # strip trailing spaces
    email = params[:person][:secondary_email]
    email = email.to_s.downcase.strip

    # Check that email is not taken
    !Email.email_available?(email, @current_community.id) || (@current_community && ! @current_community.email_allowed?(email))
  end
end
