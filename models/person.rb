...
#  one_line_desciption                :string(255)
...
#  person_type                        :integer          default("Seller")
#  first_name                         :string(255)
#  last_name                          :string(255)
#  bank_id                            :string(255)
#  branch                             :string(255)
#  account_number                     :string(255)
#  account_type                       :string(12)
#  identification_file_name           :string(255)
#  identification_content_type        :string(255)
#  identification_file_size           :integer
#  identification_updated_at          :datetime
#  tiktok_link                        :string(255)
...
#  index_people_on_bank_id                            (bank_id)
...
class Person < ApplicationRecord
  ...
  has_many :people_categories, :foreign_key => "author_id"
  has_many :categories, through: :people_categories
  ...
  validates_attachment_size :cover_photo, :less_than => 9.megabytes
  validates_attachment_content_type :cover_photo,
                                    :content_type => IMAGE_CONTENT_TYPE

  has_attached_file :identification
  validates_attachment :identification
  validates_attachment_size :identification, :less_than => 9.megabytes
  ...
  def secondary_mail
    emails.where(secondary_email: true).last
  end
